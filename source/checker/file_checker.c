...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/20 14:34:14 by sekeskin          #+#    #+#             */
/*   Updated: 2024/11/20 17:10:33 by sekeskin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_after_fields_index(char *file)
{
	...
	while (file[i])
	{
		if (is_line_field(&file[i]))
		{
			...
			return (error(62, "checker/file_checker.c", \
				"Invalid character found", NULL));
		i++;
	}
	return (error(66, "checker/file_checker.c", \
...
static	int	check_alien_chars(char *file)
{
	...
	while (file[i])
	{
		if (!(file[i] == SPACE || file[i] == ROAD
			...
			return (error(82, "checker/file_checker.c", \
				"Invalid character found", NULL));
		i++;
	}
	...
	if (start_counter != 1)
		return (error(91, "checker/file_checker.c", \
			"Invalid character found", NULL));
	return (1);
}
...
int	file_checker(char *file)
{
	...
	while (file[i] && (file[i] != ' ' && file[i] != '1' && file[i] != '0'))
	{
		if (file[i] == '\n')
			...
			return (error(108, "checker/file_checker.c", \
				"Invalid character found", NULL));
	}
	if (file[i])
	{
		while (file[i++])
			if (file[i] == '\n' && file[i + 1] == '\n')
				return (error(115, "checker/file_checker.c", \
					"There are places on the map", NULL));
	}
	else
		return (error(119, "checker/file_checker.c", \
